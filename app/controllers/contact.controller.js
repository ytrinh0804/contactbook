...
exports.findOne = async (req, res, next) => {
    ...
    const [error, documents] = await handlePromise(Contact.findOne(condition));

    if (error) {
        return next(new BadRequestError(500,
            `Erroe retrieving contact with id=${req.params.id}`));
    }
...
exports.update = async (req, res, next) => {
    ...
    const [error, document] = await handlePromise(
        Contact.findOnneAndUpdate(condition, req.body, {
            new: true,
        })
    );
    ...
    if (error) {
        return next(new BadRequestError(500,
            `Error upating contact with id=${req.params.id}`));
    }
